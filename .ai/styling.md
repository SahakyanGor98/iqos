...
- **Formatting helpers** live in `lib/utils.ts`: `formatPrice` (RU rubles, non-breaking `₽`), `fixCasing`, `formatDeviceTitle`, and `getDeviceColorSwatch(hex)` (returns a swatch background from a product's `attributes.hex` — colors are data-driven; see features.md §8. The old keyword map `DEVICE_COLOR_SWATCH_MAP` was removed). Reuse these instead of re-implementing price/title/color rendering.
