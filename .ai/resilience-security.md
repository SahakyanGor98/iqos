...
- **`'unsafe-eval'` + `wss://mc.yandex.ru` are for Metrika webvisor.** Session replay (`webvisor:true`) uses `eval`/`new Function` and a `wss://mc.yandex.ru/solid.ws` WebSocket. This is an accepted trade-off to keep webvisor; disabling webvisor (`webvisor:false`) would let us drop `'unsafe-eval'` and the `wss:` source for a stricter policy.
