...
/**
 * Resolve a swatch background from a product's `attributes.hex`. Colors are now
 * fully data-driven (backfilled by supabase/migrations/20260902_gadget_colors.sql;
 * accessories already carried hex). The former hardcoded DEVICE_COLOR_SWATCH_MAP
 * was removed. Falls back to neutral gray when a product has no hex set yet.
 * `hex` may also be a CSS gradient string (e.g. Seletti editions).
 */
export function getDeviceColorSwatch(hex?: string): { background: string; border?: string } {
  return hex ? { background: hex } : DEFAULT_SWATCH;
}
